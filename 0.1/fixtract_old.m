function FixDat = fixtract(ASL,segment,trialStartTs,trialEndTs,trialIdentifier,paramStruc,Screen,etfs,etparams)
...
if nargin < 2 & length(ASL.segData) > 1
    segment = input(sprintf('\nThere are %i segments. Which one do you want? ',length(ASL.segData)));
end

etdatH = ASL.segData(segment).horz;
etdatV = ASL.segData(segment).vert;

%Time points to discard due to out of range pupil or gaze coordinates
ETbadtimes = ASL.segData(segment).pupil > pupilRange(2) | ASL.segData(segment).pupil < pupilRange(1) ;
