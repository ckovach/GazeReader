...
function [Theta, I, LL, badcond] = logitmodelsp(R,Wsp,InitTheta, Sreg, runiter, fix )
...
elseif length(InitTheta ) < size(X,1)
...
[q,Ir,Jc] = unsparsifymex(SpX);
clear q;

%Sparse Matrix for summing within trial blocks

SEtrial = (SpX~=0)'*(SpX~=0) ~=0;
% SEtrial = makeSEtrial(b, Nvar);  %SEtrial is huge with large no of options. 
                                %Write a mex function to handle this using
                                %memcpy iterated over trials. 
...
%  for i = 1:size(X,3)
%      
% %     %Parameters associated with the outcome on each trials
%       WX(:,:,i) =  X(:,W(i),i);
% %     %Exterior products of Xs (needed later)
% %     for j = 1:size(X,2)
% %         XX(:,:,i,j) = X(:,j,i)*X(:,j,i)';
% %     end
%  end
...
dstep = Inf;
count = 0;
fprintf('\nIteration     %5i: dstep = %1.2e',count,dstep);
...
%     THsp = sparseblockmex(repmat(theta,ncols,1),Ir,Jc); %Performance can be improved with a dedicated mex function here
    ...
    dgP = sparseblockmex(full(P),0:length(P), 0:length(P),length(P)); %faster than diag(P) or diag(sparse(P))

%     D2L =  SEblk'*( wgtsumblk'*wgtsumblk- SpX*dgP*SpX')*SEblk; 

    D2L =  unsparsify( wgtsumblk'*wgtsumblk- SpX*dgP*SpX')*SEblk; %This is the slow step
...
% %         ext =  exp(  sum( Theta(:,ones(Nopt,1),ones(Ntrials ,1) ) .* X , 1 ) ) ;
% %         %extPerm =  exp(  sum( ThetaT(ones(Nopt,1),:,ones(Ntrials ,1) ) .* Xperm , 2 ) ) ;
...
% %         ext1 =  exp(  sum( Theta1(:,ones(Nopt,1),ones(Ntrials ,1) ) .* X , 1 ) ) ;
% %         ext2 =  exp(  sum( Theta2(:,ones(Nopt,1),ones(Ntrials ,1) ) .* X , 1 ) ) ;
% %     
% %         LL1 = sum(Theta1'*squeeze(WX)) - sum(log(sum(ext1)));
% %         LL2 = sum(Theta2'*squeeze(WX)) - sum(log(sum(ext2)));
        TH1 = repmat(diag(Theta1),1,nblocks);
        TH2 = repmat(diag(Theta2),1,nblocks);
        LL1 = (  sum(TH1,1) * SpX  - log(( exp( sum(TH1,1) * SpX )*SEtrial)) ) * Wsp;
        LL2 = (  sum(TH2,1) * SpX  - log(( exp( sum(TH2,1) * SpX )*SEtrial)) ) * Wsp;
...
%         LLs(end+1) = LL
    ...
    fprintf('%5i: dstep = %1.2e',count,dstep);
%         fprintf('\b\b\b\b\b%5i',count);
...
%          eigs = svd(D2L);
%          svdrat(count) = eigs(1)./eigs(end); 
         llhist(count) = LL;
         
         if count > 150 && std(llhist(end-50:end))./diff(llhist([1,end])) < 1e6 
            
             warning('Failed to converge. Estimate is likely unbounded.')
             pause(1)
             badcond = 1;
             break
         end
