...
%Reads data from sepcified sregment.
%
...
ASLdat = struct('fileInfo',struct,'allHeaderData',struct,'segData',struct);
...
if ~headerOnly
    ...
    ASLdat.segData = struct('horz',[],'vert',[],'pupil',[],'xdat',[]);
    reclbls = {RecDat(:).lbl};
    ...
    for s = segment 
        ...
        for i = 1:length(RecDat)
            fseek(fid,double(recBytePos(i) - 1 + segPosDat(s).startByte),-1);

            prec = RecDat(i).fmt;

            ASLdat.segData(s).otherDat.(RecDat(i).lbl) = fread(fid,double(segPosDat(s).nRec ),['*',prec],BytesPerRec - RecDat(i).nbytes );
        ...
        overtime = ASLdat.segData(s).otherDat.overtime_count;
        ...
        ASLdat.segData(s).horz = RecDat(strcmp(reclbls,'horz_gaze_coord')).scalef .* double(ASLdat.segData(s).otherDat.horz_gaze_coord(corrtimes));
        ASLdat.segData(s).vert = RecDat(strcmp(reclbls,'vert_gaze_coord')).scalef .* double(ASLdat.segData(s).otherDat.vert_gaze_coord(corrtimes));
        ASLdat.segData(s).pupil = RecDat(strcmp(reclbls,'pupil_diam')).scalef .* double(ASLdat.segData(s).otherDat.pupil_diam(corrtimes));
        ASLdat.segData(s).xdat = bitand( double( ASLdat.segData(s).otherDat.XDAT(corrtimes) ) , xdatMask );
        ASLdat.segData(s).otherDat = rmfield(ASLdat.segData(s).otherDat,{'XDAT','vert_gaze_coord','pupil_diam','start_of_record'});
        ASLdat.segData(s).otherDat.otCorrecter = corrtimes;  % Fields in otherDat have to be corrected for overtime 
                                                              % with ..otherDat.fieldname(ASLdat.segData(s).otherDat.otCorrecter ) 

        %Extract event markers from the xdat column
        ASLdat.segData(s).EventMark.T = find(diff([0; ASLdat.segData(s).xdat]) > 0);
        if any(ASLdat.segData(s).EventMark.T == 1)
            warning('\nXDAT signal may be erroneous at %i in segment %i', find(diff(ASLdat.segData(s).EventMark.T) == 1),s)
        end
        ASLdat.segData(s).EventMark.id = ASLdat.segData(s).xdat( ASLdat.segData(s).EventMark.T);
