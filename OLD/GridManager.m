...
gridmanagerfcns.updatePlot= @updatePlot;
gridmanagerfcns.updateFields= @updateFields;
...
if ~isappdata(parent,'binData')
    ...
    setappdata(parent,'CurrentBinGroup',1);
...
data{1}(1) = str2double(get(handles.xrange1,'String'))./screenData.res(1);
data{1}(2) = str2double(get(handles.xrange2,'String'))./screenData.res(1);
data{1}(3) = str2double(get(handles.yrange1,'String'))./screenData.res(2);
data{1}(4) = str2double(get(handles.yrange2,'String'))./screenData.res(2);


% Create a new bin group with updated bin position data

label = binData.groups(currentGroup).label;
newBinData = makeBinData( data ,'trials',binData.groups(currentGroup).activeTrials,'precedence',binData.precedence,'label',label,'type','grid');
...
plothandles = binData.groups(currentGroup).plot(binData.groups(currentGroup),0,plottype,'parent',phandles.axes2);
% hold off
