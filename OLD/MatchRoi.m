...
for i = 1:ntrials    
   ...
   RoiDat(i).Center = cat(1,imdat.trialdat(i).pos{:}) + roi_center_offset(ones(1,length(imdat.trialdat(i).pos)),:);
   ...
   stim = cat(1,imdat.trialdat(i).num{:});
...
%     end
    ...
    if ~isempty(getfix)
        fix = cat(1,FixDat.fix(getfix).(posfield))'; %Position of fixations in this trial
...
%                                           at fixation cross
        ...
        if FixRoi(i).fixdat(1).dt - FixRoi(i).trialT(1) < .750  %Assume fixation is at fixation cross when dt exceeds 750 msec        
            initfix = FixRoi(i).fixdat(1).(posfield) - FixRoi(i).fixdat(1).(shiftfield); %fixation point before the onset of the trial
        else
            initfix = FixCrossPos;
        end
...
%         rsmat = smat;
 ...
 %         Dmat = sqrt(sum(diff([ FixCrossPos;cat(1,FixDat.fix(getfix).(posfield))]',1,2).^2));
        Dmat = sqrt(sum(diff([ initfix;cat(1,FixDat.fix(getfix).(posfield))]',1,2).^2)); %Distance between fixations
        FixRoi(i).Dmat = Dmat; 
